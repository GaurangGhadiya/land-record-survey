...
import { Button, CircularProgress, Grid, InputLabel, Paper } from "@mui/material";
...
const SurveySummary = (props) => {
  ...
  const handleChangePage = (e) => {
    setPage(e)
    dispatch(onSummaryReport(filterData, e - 1, setLoading));
  }
  useEffect(() => {
    let division_list = [];
  ...
  return (
    ...
    <Layout>
     <Grid container spacing={4} style={{ flex: 1, padding: 20 }}>
      ...
      {loading == true ? <Box display={"flex"} justifyContent={"center"} alignItems={"center"} height={"90vh"}><CircularProgress /></Box> : <TableData tableData={tableData} selectedDistrict={selectedDistrict} selectedWard={selectedWard}
