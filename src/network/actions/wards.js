...
export const onWardList = (id) => {
  return async (dispatch) => {
    ...
    try {
      // const response = await axios.get(`/master-data?status=true&parentId=${id}&masterName=ward`, {});
      // let responseData = response?.data?.data
      const response = await axios.get(`/master-data?status=${encryptDataGet(`true`)}&parentId=${encryptDataGet(JSON.stringify(id))}&masterName=${encryptDataGet("ward")}`, {});
