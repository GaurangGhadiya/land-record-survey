...
const checkCookies = (context) => {
    const userName = context.req
        ? Cookies.get('land_userName', { headers: context.req.headers })
        : Cookies.get('land_userName');
    const ulb = context.req ? Cookies.get('land_ulb', { headers: context.req.headers }) : Cookies.get('land_ulb');
    return { userName, ulb };
};
